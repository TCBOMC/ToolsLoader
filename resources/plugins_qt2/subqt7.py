...
PS_NAME_IDS = {6}
RENAME_NAME_IDS = {1, 3, 4, 6, 16}
SKIP_NAME_IDS = {2, 5, 17}

# 中文语言 ID: 0x0804=简体中文, 0x0404=繁体中文
CHINESE_LANG_IDS = {0x0804, 0x0404}

# CJK Unicode 范围
CJK_PATTERN = re.compile(r'[\u4e00-\u9fff\u3400-\u4dbf]')

# 新建中文名称记录时的目标平台
# (platformID, platEncID, langID, [nameID列表])
CHINESE_NEW_RECORDS = [
    (3, 1, 0x0804, [1, 4, 16]),  # Windows / Unicode BMP / 简体中文
    (3, 1, 0x0404, [1, 4, 16]),  # Windows / Unicode BMP / 繁体中文
    (1, 25, 0x0021, [1, 4]),  # Mac / 简体中文 / nameID 16 通常 Mac 不用
]
...
class SubtitleExtractorApp(QObject):
    ...
    def create_widgets(self):
        ...
        # 统一ttf
        self.unify_ttf_cb = QCheckBox("统一ttf")
        self.unify_ttf_cb.setChecked(False)
        self.unify_ttf_cb.setVisible(False)  # 初始隐藏
        # self.unify_ttf_cb.setFixedWidth(int(55 * self.scale))

        # 字体模式
        self.font_mode_cb = QComboBox()
        self.font_mode_cb.addItems(["封装字体", "子集合并", "字体名还原", "无处理"])
        self.font_mode_cb.setFixedWidth(int(90 * self.scale))
        ...
        # 绑定字体模式变化信号，控制 "统一ttf" 显示
        self.font_mode_cb.currentIndexChanged.connect(self.update_unify_ttf_visibility)
        ...
        self.button_layout.addWidget(self.unify_ttf_cb)
        self.button_layout.addWidget(self.font_mode_cb)

        self.main_layout.addWidget(self.button_frame)

        # ---------- 分割面板 ----------
        self.paned, main_frames = self.app.kit.ui.create_split_frame(self.root, orient="horizontal", n=2, sashwidth=5,
                                                                     sashrelief="line")
        ...
        left_container, left_tree, left_files = self.app.kit.ui.create_tree_view(left_frame, tree_index=1,
                                                                                 extra_columns=extra_cols_left)
        ...
        right_container, right_tree, right_files = self.app.kit.ui.create_tree_view(right_frame, tree_index=2,
                                                                                    extra_columns=extra_cols_right)
        ...
        self.update_unify_ttf_visibility()
    ...
    def update_unify_ttf_visibility(self):
        """根据 font_mode_cb 的值决定 unify_ttf_cb 是否显示"""
        # 只有选择 '子集合并' 才显示
        if self.font_mode_cb.currentText() == "封装字体":
            self.unify_ttf_cb.setVisible(True)
        else:
            self.unify_ttf_cb.setVisible(False)
    ...
    def extract_single_subtitle(self, fullpath, filename, stream, subfmt, outdir,
        ...
        try:
            if can_copy:
                ...
                outpath, mapping = self.handle_sup_conversion(fullpath, filename, stream, outdir, temp_font_dir, width,
                                                              height,
                                                              fps)
            ...
            try:
                ...
                self.app.kit.ui.show_message_box("提取失败",
                                                 f"轨道 {idx} 文件 {outfilename} 与备用格式导出均失败。\n错误: {e2}",
                                                 "error")
                return None, mapping

        if subfmt in ("ass", "ssa") and original_fmt in ("ass", "ssa"):
            mapping = self.extract_font_mapping(outpath)  # 先提取映射

        # 后处理逻辑（ASS字体修复、字体还原）
        if self.subfmt_cb.currentText() and subfmt in (
        "ass", "ssa") and font_mode != "无" and self.ass_fix_cb.isChecked():
            self.fix_ass_header(outpath)

        if subfmt in ("ass", "ssa") and font_mode in ("子集合并", "字体名还原"):
            try:
                # mapping = self.restore_ass_fonts(outpath)
                self.restore_ass_with_mapping(outpath, mapping)  # 再还原字体
    ...
    def handle_sup_conversion(self, fullpath, filename, stream, outdir, temp_font_dir, width, height, fps):
        ...
        # 提取临时 ASS
        ass_path, mapping = self.extract_single_subtitle(
            fullpath=fullpath,
            filename=filename,
            stream=stream,
            subfmt="ass",
    ...
    def map_font_files_to_mapping(self, folder_path, mapping):
        """
        将字体文件映射到字体ID和字体名

        参数:
        folder_path: 字体文件所在文件夹路径
        mapping: 字体映射字典 {字体ID: 字体名}

        返回:
        tuple: (id_to_path_dict, name_to_path_dict)
          - id_to_path_dict: {字体ID: 字体文件路径}
          - name_to_path_dict: {字体名: 字体文件路径}
        """
        id_to_path = {}
        name_to_path = {}
        ...
        if not os.path.exists(folder_path):
            ...
            return id_to_path, name_to_path
        ...
        for filename in os.listdir(folder_path):
            ...
            # 在mapping的键中查找匹配
            for font_id, font_name in mapping.items():
                # 匹配mapping中的键（字体ID）
                if last_part == font_id:
                    id_to_path[font_id] = file_path
                    name_to_path[font_name] = file_path
                    break
                # 也可以考虑匹配mapping中的值（字体名）
                elif last_part == font_name:
                    # 注意：这种情况下可能需要反转查找
                    # 但根据需求描述，我们只匹配mapping中的键
                    pass
    ...
    def embed_fonts_to_ass(self, ass_path, font_dir):
        """
        将 font_dir 下的字体 embed 到 ASS 字幕中
        支持 TTF/OTF
        如果 self.ass_fix_cb.isChecked()，OTF 会尝试转换为 TTF
        临时转换文件会在函数退出时自动清理
        """
        ...
        temp_dir = os.path.join(font_dir, "__temp_fonts__")
        os.makedirs(temp_dir, exist_ok=True)

        def encode_font_bytes(data: bytes) -> str:
            """优化后的 NumPy 向量化编码，速度更快。"""
            if not data:
                return ""

            # 1. 从 bytes 创建 uint8 数组（无拷贝）
            ...
            # else: pad == 0, 直接使用 d 的视图，无需拷贝

            # 3. 重塑为 Nx3 矩阵（返回视图，无拷贝）
            d = d.reshape(-1, 3)
            b1, b2, b3 = d[:, 0], d[:, 1], d[:, 2]

            # 4. 预分配输出数组
            out = np.empty(len(d) * 4, dtype=np.uint8)

            # 5. 使用 out 参数进行原地计算，避免临时数组和类型转换
            np.right_shift(b1, 2, out=out[0::4])  # out[0::4] = b1 >> 2
            np.bitwise_or(
                np.left_shift(np.bitwise_and(b1, 0x3), 4),
                np.right_shift(b2, 4),
                out=out[1::4]  # out[1::4] = ((b1 & 0x3) << 4) | (b2 >> 4)
            )
            np.bitwise_or(
                np.left_shift(np.bitwise_and(b2, 0xF), 2),
                np.right_shift(b3, 6),
                out=out[2::4]  # out[2::4] = ((b2 & 0xF) << 2) | (b3 >> 6)
            )
            np.bitwise_and(b3, 0x3F, out=out[3::4])  # out[3::4] = b3 & 0x3F
            ...
            # 7. 加偏移并转换为字符串
            out += 33
            # 尝试使用 memoryview 或 bytes 转换，可能略快
            result = memoryview(out).tobytes().decode("latin-1")
        ...
        """def encode_font_bytes(data: bytes) -> str:
            ...
            for i in range(0, len(data), 3):
                ...
                encoded.extend(chr(values[j] + 33) for j in range(count))
        ...
        try:
            for fpath in font_files:
                ...
                # 如果是 OTF 并且复选框勾选，则尝试转换
                if ext == ".otf" and self.unify_ttf_cb.isChecked():
                    ttf_path = os.path.join(temp_dir, os.path.splitext(os.path.basename(fpath))[0] + ".ttf")
                    try:
                        converted_path = self.convert_otf_to_ttf(fpath, ttf_path)
                    except Exception as e:
                        self.log(f"⚠️ OTF 转 TTF 失败，使用原 OTF: {fpath}, 错误: {e}")
                        converted_path = None
                    use_path = converted_path if converted_path and os.path.exists(converted_path) else fpath
                else:
                    use_path = fpath

                fname = os.path.basename(use_path)
                print(f"字体:{fname}")
                with open(use_path, 'rb') as fb:
                    b = fb.read()
            ...
            # 移除已有的 [Fonts] 块
            HEADER_RE = r"^\[[A-Za-z0-9 _+\-]{1,60}\]\s*$"
            text = re.sub(
                rf"(?mis)^\[fonts\]\s*$.*?(?=^{HEADER_RE}|\Z)",
                "",
                text
            ).rstrip() + "\n"

            # 插入到 [Events] 前
            match = re.search(r"(?mi)^\[events\]", text)

            if match:
                insert_pos = match.start()
                text = text[:insert_pos] + font_block + "\n" + text[insert_pos:]
            else:
                # 如果没有 [Events]，退化为追加到末尾
                text += "\n" + font_block
            ...
            if os.path.exists(temp_dir):
                try:
                    ...
                    self.log(f"🗑️ 临时目录已清理: {temp_dir}")
                except Exception as e:
                    self.log(f"⚠️ 临时目录清理失败: {e}")

    def extract_font_mapping(self, filepath):
        """
        从ASS文件中提取字体映射关系
        返回字典 {子集化字体名: 原字体名}
        """
        ...
        # 更宽松 & 健壮的正则
        pattern_subset_comment = re.compile(
            r";\s*Font\s*Subset\s*:\s*([A-Za-z0-9]+)\s*[-–—]\s*(.+)",
            re.IGNORECASE
        )

        pattern_assfonts = re.compile(
            r"(.+?)\s*----\s*([A-Za-z0-9]+)"
        )
        ...
        try:
            with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for raw_line in f:
                    ...
                    if line.startswith("[") and line.endswith("]"):
                        in_assfonts_section = (line.lower() == "[assfonts rename info]")
                        continue

                    # ===== 第一种格式 =====
                    m1 = pattern_subset_comment.search(line)
                    if m1:
                        subset, realname = m1.groups()
                        subset = subset.strip()
                        realname = realname.strip()

                        if subset and realname:
                            mapping[subset] = realname
                        continue

                    # ===== 第二种格式（仅在区块内）=====
                    if in_assfonts_section:
                        m2 = pattern_assfonts.search(line)
                        if m2:
                            realname, subset = m2.groups()
                            subset = subset.strip()
                            realname = realname.strip()
            ...
            print(f"[WARN] 解析字体映射失败: {e}")

        return mapping

    def restore_ass_with_mapping(self, filepath, mapping):
        """
        使用字体映射关系还原ASS文件中的字体名
        - 替换 [V4+ Styles] 中的字体
        - 替换 Dialogue 行中 {} 内的字体
        """
    ...
    def normalize_to_ascii(self, name: str) -> str:
        """生成符合 PostScript 名称规范的 ASCII 字符串"""
    ...
    def replace_font_name_complete(self, font_path, old_name, new_name, output_path=None, font_index=0):
        """
        重命名字体所有名称记录。
        old_name 保留用于 API 兼容，实际不再需要——直接设置所有可重命名的 nameID。
        新增 font_index 参数用于 TTC 集合字体，默认 0。
        """
        if not os.path.exists(font_path):
            return ""

        try:
            ext_lower = os.path.splitext(font_path)[1].lower()
            if ext_lower == '.ttc':
                ttc = TTCollection(font_path)
                font = ttc.fonts[font_index]
            else:
                font = TTFont(font_path)

            name_table = font['name']
            ascii_name = self.normalize_to_ascii(new_name)
            use_chinese = bool(re.search(r'[\u4e00-\u9fff\u3400-\u4dbf]', new_name))
            chinese_name = new_name if use_chinese else None

            # 判断是否已有任何平台的中文 Family Name 记录
            chinese_lang_ids = {0x0804, 0x0404}
            has_chinese_family = any(
                r.nameID == 1 and r.langID in chinese_lang_ids
                for r in name_table.names
            )
            need_create_chinese = use_chinese and not has_chinese_family
            ...
            for record in name_table.names:
                ...
                if nameID in skip_name_ids or nameID not in rename_name_ids:
                    continue

                # PostScript 名始终 ASCII，其他直接用目标名称
                new_val = ascii_name if nameID in ps_name_ids else new_name

                # 用该记录自身的编码写入，编码失败回退 ASCII
                enc = record.getEncoding()
                try:
                    record.string = new_val.encode(enc)
                except (UnicodeEncodeError, LookupError):
                    record.string = ascii_name.encode('ascii')

                name_records_dict.setdefault(nameID, {})[record.platformID] = record.string

            # 新建中文名称记录（多平台）
            if need_create_chinese:
                for pid, eid, lid, name_ids in [
                    (3, 1, 0x0804, [1, 4, 16]),  # Windows 简体中文
                    (3, 1, 0x0404, [1, 4, 16]),  # Windows 繁体中文
                    (1, 25, 0x0021, [1, 4]),  # Mac 简体中文
                ]:
                    for nid in name_ids:
                        try:
                            name_table.setName(chinese_name, nid, pid, eid, lid)
                        except Exception:
                            pass

            # 更新 CFF 表（OTF）
            if 'CFF ' in font:
                font['CFF '].cff.fontNames = [ascii_name]
        ...
        for base_name, font_files in font_groups.items():
            ...
            try:
                if len(font_files) == 1:
                    ...
                    # 修复 name 表
                    font_basename = os.path.basename(src)
                    ...
                    success_count += 1
                    self.log(f"✅ 处理字体: {base_name} (单个文件)")
...
'''
    ...
    def convert_otf_to_ttf(self, input_path, output_path):
        """
        使用 FontForge 将 OTF 转换为 TTF，并实时输出信息
        :param input_path: 输入 OTF 路径
        :param output_path: 输出 TTF 路径
        :return: 转换后的 TTF 路径 或 None
        """
        input_path = str(input_path)
        output_path = str(output_path)

        if not os.path.exists(input_path):
            self.log(f"❌ 输入文件不存在: {input_path}")
            return None
...
print("🔄 开始转换字体: {input_path!r}")
sys.stdout.flush()

try:
    font = fontforge.open(r"{input_path}")
    font.encoding = "UnicodeFull"
    font.generate(r"{output_path}")
    font.close()
    print("✅ 字体保存成功: {output_path!r}")
    sys.stdout.flush()
except Exception:
    print("❌ 转换过程中发生异常")
    traceback.print_exc()
...
'''
        ...
        try:
            ...
            if proc.returncode != 0:
                self.log(f"❌ FontForge 返回码非零: {proc.returncode}")
                return None
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                self.log(f"⚠️ 输出文件未生成或为空: {output_path}")
                return None
    ...
    def set_buttons_state(self, state: str):
        ...
        self.unify_ttf_cb.setEnabled(enabled)
        # 如果你将来想控制 tree，也可以类似：
    ...
    def save_and_disable_buttons(self):
        ...
        self._original_states = {
            ...
            'unify_ttf_cb': self.unify_ttf_cb.isEnabled(),
    ...
    def restore_buttons_state(self):
        ...
        self.unify_ttf_cb.setEnabled(self._original_states.get('font_mode_cb', True))
...
def get_info():
    return {"icon": {"text_ico": "\uf20a"}, "display_name": "字幕提取器"}
