...
def create_ui(main, canvas_frame, button_frame, scale):
    ...
    splitter, subframes = main.kit.ui.create_split_frame(
        orient="horizontal", n=3, sashwidth=3, handle_color="#d0d0d0", bg="#f0f0f0", sashrelief="line"
    )
    ...
    left_frame, center_frame, right_frame = subframes
    ...
    # Center Column 内容
    # -------------------------
    center_container = QFrame(center_frame)
    center_container.setObjectName("center_container")
    center_container.setFrameShape(QFrame.NoFrame)  # 不使用默认边框
    center_container.setAutoFillBackground(True)

    # 精准设置 container 背景和边框，不影响内部控件
    ...
    # 放入 center_frame
    outer_layout = QVBoxLayout(center_frame)
...
def get_info():
    return {"icon":{"text":"📆"}, "display_name":"测试UI"}