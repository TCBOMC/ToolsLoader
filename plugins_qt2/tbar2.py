...
def create_ui(external_main, canvas_frame, button_frame, scale):
    ...
    tree_c1, tree1, tree_list1 = main.kit.ui.create_tree_view(left_frame, tree_index=3, extra_columns=extra_cols1, mode="表格", show_checkbox=False)
    tree_c1.show()
    ...
    extra_cols = [
        ...
        ("高度", 50, False, 2, "combobox(选项1|选项2|选项3)", [
        lambda idx, row, widget, col, ts=test_s: print(f"测试文本：{ts},按钮点了 row={row}, col={col},按钮为：{widget}，选中索引:", idx)]),
        ("帧率", 50, False, 4, "button(打印)", [
        main.kit.mathkit.print_main_title,
        lambda widget: print(f"第二个回调执行:{widget}")])
    ]
    tree_c, tree, tree_list = main.kit.ui.create_tree_view(tree_frame, tree_index=1, extra_columns=extra_cols, mode="表格")
    tree_c.show()
    ...
    tree_c1, tree1, tree_list1 = main.kit.ui.create_tree_view(center_frame, tree_index=2, extra_columns=extra_cols1, mode="文件")
    tree_c1.show()
    ...
    # 启动定时器，每隔2秒调用一次 toggle_tree_checkbox(2)
    start_toggle_timer()
...
def test_add_random_items(tree_id=1, count=5):
    ...
    for _ in range(count):
        ...
        content = {
            ...
            None: fname,    # 文件名列
            "宽度": f"宽{width}",    # extra 列
            "高度": f"高{height}",   # extra 列
            "帧率": f"帧{frame}",
        }
        """content = {
            0: checked,  # 复选框列
...
def toggle_checkbox_callback():
    """定时器回调函数，调用toggle_tree_checkbox(2)"""
    global _timer_running

    # 检查是否应该继续运行
    if not _timer_running or main is None:
        print("定时器已停止，不再继续")
        return
    ...
    try:
        if main is not None:
            ...
            #print(f"复选框显示状态: {is_visible}")
    except Exception as e:
        print(f"调用 toggle_tree_checkbox 时出错: {e}")
    finally:
        # 只有在标志为True时才继续下一次调用
        if _timer_running and main is not None:
            QTimer.singleShot(2000, toggle_checkbox_callback)


def start_toggle_timer():
    """启动定时器"""
    global _timer_running
    # 停止之前的定时器
    _timer_running = False  # 先停止旧的

    # 启动新的定时器
    _timer_running = True
    QTimer.singleShot(2000, toggle_checkbox_callback)
    print("定时器已启动，每隔2秒调用 toggle_tree_checkbox(2)")