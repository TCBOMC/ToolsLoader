...
class CollapsibleSideBar(QWidget):
    ...
    def toggle_side_bar(self):
        ...
        # 启动手动动画
        self.animate_sidebar(start, end, duration=300)
        ...
        self.toggle_btn.setToolTip("收起" if self.expanded else "展开")
        self.update_toolbar_state()
        self.update_button_margins()

    def animate_sidebar(self, start, end, duration=300):
        self.animating = True
        ...
        def update():
            ...
            # 强制宽度
            self.side_bar.setMinimumWidth(int(value))
            self.side_bar.setMaximumWidth(int(value))

            if progress >= 1:
                self.side_bar.setMinimumWidth(end)
                self.side_bar.setMaximumWidth(end)

                self.anim_timer.stop()
                self.animating = False

        self.anim_timer = QTimer()
        self.anim_timer.timeout.connect(update)
